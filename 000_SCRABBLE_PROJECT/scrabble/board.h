...
public:
    ...
    // Note: These methods have been made public for HW 5
    bool is_in_bounds(const Position& position) const;
    bool in_bounds_and_has_tile(const Position& position) const;

    PlaceResult opposite_direction_checker(const Move& move) const;
...
private:
    ...
    // bool is_in_bounds(const Position& position) const; NOW PUBLIC
    // bool in_bounds_and_has_tile(const Position& position) const; NOW PUBLIC
